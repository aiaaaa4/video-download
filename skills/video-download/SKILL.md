...
- If stdout is `first-use-required`, send exactly: `首次使用video-download skill，我将执行一次依赖/环境检查和更新，后续任务将跳过此步骤。` Then run `python scripts/setup_check.py`. Continue only after it succeeds, and report the resulting installed versions of both yt-dlp and FFmpeg plus the yt-dlp update status to the user.
...
Use `--merge-format mp4` only after selecting MP4-compatible video and playback-audio codecs. Otherwise keep MKV. If `--list-subs` shows a usable manual track in `SOURCE_LANG`, use `--subtitle-kind manual`; use `auto` only when no manual track exists. This Skill downloads the source subtitle but does not perform ASR.
