...
2. 下载清单：视频文件 1 份保存在项目根目录；播放音频和 ASR 音频各 1 份、原语言字幕 1 份（如有，默认 SRT）、原始封面图 1 份（如有，默认平台最高质量并转为 PNG）作为配套素材，保存在项目的 .work/input/ 中。播放音频同时用于与视频流合并，ASR 音频独立选择最适合转写的最高质量源音频。
3. 视频质量：默认选择第 1 项，即最高可用 SDR 兼容方案。实际可用选项如下；每项应包含分辨率、编码、容器和可见的估算大小：
{options}
4. 文件命名：默认名称为“{safe_field(default_name)}”，顺序是原语言真实标题、平台日期、尾部视频 ID。用户也可以提供自定义名称；自定义名称会同时用于项目文件夹、视频、两份音频和原语言字幕。
