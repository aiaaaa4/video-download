...
这个 Skill 使用 `yt-dlp` 与 FFmpeg，在下载前探测并列出实际分辨率、编码和可见的估算大小，再确认下载清单、画质、命名、保存位置、源语言和播放列表行为。默认选择最高可用 SDR 兼容方案，下载纯视频流和最佳播放音轨并合并为成片；合并后删除这两份临时文件，只在项目根目录保留成片。另行选择的 ASR 最佳源音频以“`视频名.ASR校对音频.<原始扩展名>`”保存，和最多一份 SRT 原语言字幕及平台最高质量 PNG 封面一起放入 `.work/input/`。默认在桌面下新建独立项目文件夹，并以“原语言真实标题、平台日期、尾部视频 ID”统一命名项目和文件。
