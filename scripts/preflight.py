...
COMBINED = """此工作流由 3 个视频 Skill 组成，可一键识别链接并下载视频文件、翻译字幕、完成视频封装等任务。最终项目工程文件夹默认包含 2 个视频文件（下载原版及发布版）、3 个字幕文件（双语 ASS、双语 SRT 和发布版双语 BCC）以及 1 个原始封面文件；如开启抽帧封面，还会额外生成 5 张封面候选。
...
开始“视频下载 → 字幕翻译 → 视频封装”组合流程前，请先准备固定转写服务的凭据：
...
11. 发布封装：默认添加 3 秒免责声明，并根据双语 SRT 生成时间轴校准后的发布版双语 BCC；默认关闭抽帧封面，不加水印、不烧录字幕、不裁切、不加滤镜，也不重新编码视频主体。如需抽取 5 张封面候选，请回复“开启抽帧封面”。
12. 发布版视频位置：默认保存在原视频项目文件夹，并命名为“原视频名.发布版.mp4”。
13. 文件覆盖：默认不覆盖任何已存在的同名文件；如允许覆盖，请明确说明。
